...
# Accumulate original + BT data over time and train cumulatively
used_hashes = set()
accumulated_src = []
accumulated_tgt = []
...
for epoch in range(1, 6):  # Epochs 1 to 5
    ...
    # Use cross-entropy to select high-quality dev subset
    selected_dataset = cross_entropy_difference(current_model_name, 'gpt2', train_dataset['dev'])

    # Deduplicate and extract new examples
    new_examples = []
    ...
    # Combine with original parallel data
    ...
    reduced_dataset = combined_dataset  
    ...
    results_per_epoch[epoch] = {"comet": comet_score, "bleu": bleu_score}
...
#     epoch_slice = original_dataset.select(range(0, end_idx))  # cumulative slice

#     print(f"Epoch {epoch}: Training on cumulative {len(epoch_slice)} original samples out of {len(original_dataset)} total ({len(epoch_slice) / len(original_dataset):.2%})")
...
#     current_model_name = epoch_output_dir  # For next round
...
#     print(f"Epoch {epoch} (Original Only Cumulative) -> COMET: {comet_score:.2f}, BLEU: {bleu_score:.2f}")

# # Summary
# print("\n=== Original Only Cumulative Results ===")
...
# for epoch in range(1, 6):
#     print(f"\n=== Epoch {epoch}/5 — Cumulative BT Only ===")

#     # Add current slice to cumulative indices
#     start_idx = (epoch - 1) * slice_size
#     end_idx = epoch * slice_size
#     cumulative_indices.extend(range(start_idx, end_idx))

#     # Select cumulative data
#     epoch_slice = bt_dataset.select(cumulative_indices)
...
#     print(f"Epoch {epoch} (BT Only Cumulative) -> COMET: {comet_score:.2f}, BLEU: {bleu_score:.2f}")

# # Summary
# print("\n=== BT Only Cumulative Results ===")
